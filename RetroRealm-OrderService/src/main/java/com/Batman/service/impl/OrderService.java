...
public class OrderService implements IOrderService {
	...
	public Order placeOrder(OrderRequest orderRequest, BindingResult bindingResult) {
		...
		CartValueResponse userCartValue = getCartResponse(userId);
		...
		Order orderEntity = Order.builder().orderItems(orderRequest.getGameIds()).userId(orderRequest.getUserId())
				.orderPrice(orderRequest.getTotalPrice()).paymentType(orderRequest.getPaymentType()).orderStatus(OrderStatus.INITIATED).build();
		...
		OrderDetails orderDetails = OrderDetails.builder().gameIds(order.getOrderItems())
				...
				.totalPrice(order.getOrderPrice()).userId(order.getUserId()).paymentType(order.getPaymentType()).build();

		//Assuming the Payment Details are already available
		//If no this event no needs to be published 
		CompletableFuture<SendResult<String,OrderDetails>> future = kafkaTemplate.send(KafkaConstants.TOPIC, orderDetails);
	...
	public Order getOrderById(Integer orderId) {
		log.info("Entering Get Order By Id in Order Servicde...");
